...
class TrestleReversePhoneScanner(Scanner):
    ...
    description = (
        ...
        f"({TRESTLE_REVERSE_PHONE_URL}) when TRESTLE_API_KEY is set. Emits "
        "owner names, current/associated addresses, emails, and brief line "
        "metadata only when Trestle returns them. No key → skipped, no HTTP."
    )
    accepts = [QueryType.phone]
    optional_key = "TRESTLE_API_KEY"
    limitations = (
        "Requires TRESTLE_API_KEY (~$0.07/query PAYG). Without it this module "
        "is skipped. Phone queries only. Missing fields stay empty — nothing "
        "is invented. Does not scrape Whitepages/Spokeo and does not return "
        "breach data."
    )
    timeout = 15.0
    ...
    async def run(self, query: Query) -> ScannerResult:
        ...
        number = (query.phone_e164 or "").strip()
        if not number and query.type == QueryType.phone:
            number = (query.raw or "").strip()
        if not number:
